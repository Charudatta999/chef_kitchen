...
namespace LinkedList
{
    ...
    DoubleLL* DoublelyLinkedList::GetNode(const std::string& val)
    {
        ...
        if (val == head_->val_) return head_;
        if (val == tail_->val_) return tail_;
        ...
        {
            if (start->val_ == val) return start;
            if (end->val_ == val) return end;
            start = start->next_;
            end = end->prev_;
        }
        if (start != nullptr && start == end && start->val_ == val) return start;
        return nullptr;
    }
    ...
    DoubleLL* DoublelyLinkedList::GetNode(int key)
    {
        ...
        if (start == end) 
        {
            if (start->key_ == key) return start;
            return nullptr;
        }
    ...
    bool DoublelyLinkedList::SwapNodes(DoubleLL* first, DoubleLL* second)
    {
        ...
        if(first->next_ == second) {
            ...
            // Adjust head and tail if necessary
            if (head_ == second) head_ = first;
            if (tail_ == first) tail_ = second;
    
            return true;
        } 
        else {
            // Non-adjacent case: first and second are not adjacent
            if (head_ == first) head_ = second; // If first is at head, update head
            else if (head_ == second) head_ = first; // If second is at head, update head
    
            if (tail_ == first) tail_ = second; // If first is at tail, update tail
            else if (tail_ == second) tail_ = first; // If second is at tail, update tail
            ...
            if (firstPrev) firstPrev->next_ = second;
            if (firstNext) firstNext->prev_ = second;
            if (secondPrev) secondPrev->next_ = first;
            if (secondNext) secondNext->prev_ = first;
